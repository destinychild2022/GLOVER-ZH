...
DATASET_DIR="/mnt/data-cpfs/workspace_xl/code/GLOVER-ZH/SAM_finetune/dataset"  # 保留用于兼容性，实际不使用
IMAGES_DIR="/mnt/data-cpfs/workspace_xl/code/GLOVER-ZH/SAM_finetune/dataset/picture"  # 父目录，代码会自动遍历子目录

# 设置输出目录
OUTPUT_DIR="/mnt/data-oss/rap-prod-bak/GLOVER/output/sam_finetuned_robot_arm_point2"
...
# 检查mask目录
MASKS_DIR="/mnt/data-cpfs/workspace_xl/code/GLOVER-ZH/SAM_finetune/dataset/mask"
if [ ! -d "$MASKS_DIR" ]; then
...
python "finetune_sam_lora_point.py" \
    ...
    --swanlab_project "SAM-Finetune-RobotArm" \
