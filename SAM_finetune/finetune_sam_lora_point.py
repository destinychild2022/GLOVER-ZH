...
def train_epoch(model, dataloader, optimizer, scaler, device, epoch, loss_weights, swanlab_run=None):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # 使用混合精度训练
        with autocast():
            # 使用SAM的forward方法（参考Sam_LoRA）
            outputs = actual_model(batched_input=batched_input, multimask_output=False)
...
def main():
    ...
    base_masks_dir = "/mnt/data-cpfs/workspace_xl/code/GLOVER-ZH/SAM_finetune/dataset/mask"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 验证（带可视化）
        visualize_dir = "/mnt/data-cpfs/workspace_xl/code/GLOVER-ZH/SAM_finetune/backup/visualize/point2"
